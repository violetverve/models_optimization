...
# with addition of steaming with gstreamer with openvas 
...
def detect(opt):
    ...
    gst_str = (
        f'appsrc ! queue ! videoconvert ! videoscale ! videorate ! video/x-raw,framerate=30/1 '
        f'! vp8enc deadline=1 target-bitrate=5000000 cpu-used=5 end-usage=cbr '
        f'! rtpvp8pay ! queue ! udpsink host={host_ip} port=5000'
    )

    out = cv2.VideoWriter(gst_str, cv2.CAP_GSTREAMER, 0, 30, (int(cap.get(cv2.CAP_PROP_FRAME_WIDTH)), int(cap.get(cv2.CAP_PROP_FRAME_HEIGHT))), True)

    print("CAP: " + str(int(cap.get(cv2.CAP_PROP_FRAME_WIDTH))) +" " + str(int(cap.get(cv2.CAP_PROP_FRAME_HEIGHT))) )

    if not out.isOpened():
        print("Error: Unable to open video writer with GStreamer pipeline.")
        return
